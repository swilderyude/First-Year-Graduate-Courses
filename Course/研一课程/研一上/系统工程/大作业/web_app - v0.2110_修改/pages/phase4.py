...
def build_model_context_alert():
    ...
    # 4. 返回 Alert
    return dbc.Alert([
        html.H6([html.Span("🗄️", className="me-2"), "可用变量池 (Phase 3 Design Space)"], className="alert-heading"),
        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    # 动态容器
                    dbc.Alert([
                        html.H6([html.Span("🗄️", className="me-2"), "可用变量池 (点击图标复制)"], className="alert-heading"),
                        html.Hr(),
                        html.Div([
                    ...
                    dbc.Row([
                        dbc.Col([
                            dbc.Button([html.Span("✓", className="me-2"),"验证代码"], id="btn-validate-attr", color="success", size="sm", className="w-100")
                        ], md=3),
                        dbc.Col([
                            dbc.Button([html.Span("▶️", className="me-2"),"测试执行"], id="btn-test-attr", color="info", size="sm", className="w-100")
                        ], md=3),
                        dbc.Col([
                            dbc.Button([html.Span("💾", className="me-2"),"保存函数"], id="btn-save-attr", color="warning", size="sm", className="w-100")
                        ], md=3),
                ...
                dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            dbc.Row([
                                dbc.Col([
                                    dbc.Button([html.Span("✓", className="me-2"),"验证代码"], id="btn-validate-utility", color="success", size="sm", className="w-100")
                                ], md=3),
                                dbc.Col([
                                    dbc.Button([html.Span("▶️", className="me-2"),"测试执行"], id="btn-test-utility", color="info", size="sm", className="w-100")
                                ], md=3),
                                dbc.Col([
                                    dbc.Button([html.Span("💾", className="me-2"),"保存函数"], id="btn-save-utility", color="warning", size="sm", className="w-100")
                                ], md=3),
...
html.Label("Python代码编辑器:", className="fw-bold"),
                            ...
                            dbc.Row([
                                dbc.Col([
                                    dbc.Button([html.Span("✓", className="me-2"),"验证代码"], id="btn-validate-weights-mau", color="success", size="sm", className="w-100")
                                ], md=3),
                                dbc.Col([
                                    dbc.Button([html.Span("▶️", className="me-2"),"测试执行"], id="btn-test-weights-mau", color="info", size="sm", className="w-100")
                                ], md=3),
                                dbc.Col([
                                    dbc.Button([html.Span("💾", className="me-2"),"保存代码"], id="btn-save-weights-mau", color="warning", size="sm", className="w-100")
                                ], md=3),
    ...
    try:
        ...
        if perf_models:
            for name in perf_models.keys():
                attr_badges_list.append(dbc.Badge([html.Span("✓", className="me-1"), name], color="success", className="me-1 mb-1 p-2"))
        ...
        if utility_funcs:
            for name in utility_funcs.keys():
                util_badges_list.append(dbc.Badge([html.Span("✓", className="me-1"), name], color="info", className="me-1 mb-1 p-2"))
        ...
        if ctx.triggered_id == "btn-load-phase4":
            status_msg = dbc.Alert(["✓", "数据与草稿已加载"], color="success", duration=4000)
...
def test_attr_execution_real_data(n_clicks, code, metric):
    ...
    try:
        ...
        results_ui = [
            html.Div(
                [html.Span("🕐", className="me-1"), f"Test Run: {time_str}"], 
