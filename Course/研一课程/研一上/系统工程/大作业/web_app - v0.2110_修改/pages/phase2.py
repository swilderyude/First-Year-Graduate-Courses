        ...
        dbc.Col([
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    dcc.Graph(id="n2-diagram", figure=go.Figure(),
...
def display_components(components):
    ...
    for idx, comp in enumerate(components):
        ...
        items.append(
            dbc.ListGroupItem([
                html.Div([
                    html.Span(f"{type_icons.get(comp['type'], '📦')} ", className="me-2"),
...
def auto_save_phase2_ui(comp_name, comp_type, comp_desc, 
    ...
    updates['timestamp'] = str(pd.Timestamp.now())
    ...
    try:    
        ...
        if triggered_by_button:
            ...
            if count_c > 0 or count_i > 0:
                status_msg = dbc.Alert(["✓", f"已加载：{count_c}组件, {count_i}接口"], color="success")
            else:
