...
def serve_tutorial():
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    html_content = f"<!DOCTYPE html><html><head><meta charset='UTF-8'><title>系统工程分析平台 - 使用教程</title><style>body{{font-family:Arial,sans-serif;max-width:800px;margin:50px auto;padding:20px;line-height:1.6}} h1{{color:#007bff;border-bottom:2px solid #007bff;padding-bottom:10px}} h2{{color:#28a745;margin-top:30px}} h3{{color:#17a2b8;margin-top:20px}} ul{{margin-left:20px}} li{{margin-bottom:8px}} hr{{border:1px solid #ccc;margin:30px 0}} pre{{background-color:#f8f9fa;padding:15px;border-radius:5px;overflow-x:auto}}</style></head><body><pre style='white-space:pre-wrap;background:none;padding:0;'>{content}</pre></body></html>"
...
app.layout = dbc.Container([
    ...
    dbc.Navbar(
        dbc.Container([
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.Span("🛰️", className="fa-2x me-3"),
    ...
    dcc.Store(id='phase1-auto-load-trigger', storage_type='session'),
    dcc.Store(id='phase2-auto-load-trigger', storage_type='session'),  # 新增：Phase 2自动加载触发器
